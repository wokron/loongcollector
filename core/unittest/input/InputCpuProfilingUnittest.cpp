...
void InputCpuProfilingUnittest::OnSuccessfulStart() {
    ...
    std::string serverPipelineName = ebpf::EBPFServer::GetInstance()->checkLoadedPipelineName(logtail::ebpf::PluginType::CPU_PROFILING);
    std::string pipelineName = input->GetContext().GetConfigName();
    APSARA_TEST_TRUE(serverPipelineName == pipelineName);
...
void InputCpuProfilingUnittest::OnSuccessfulStop() {
    ...
    APSARA_TEST_TRUE(serverPipelineName == pipelineName);
    APSARA_TEST_TRUE(input->Stop(true));
    serverPipelineName = ebpf::EBPFServer::GetInstance()->checkLoadedPipelineName(logtail::ebpf::PluginType::CPU_PROFILING);
